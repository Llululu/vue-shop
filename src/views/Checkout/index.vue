...
const getCheckoutInfo = async () => {
  ...
  // 筛选默认地址
  const defaultAdd = checkInfo.value.userAddresses.find(item => item.isDefault === 0)
  curAddress.value = defaultAdd
}
...
const changeAddress = (add) => {
  selectedAdd.value = add
}

// 确认按钮点击事件
const confirm = () => {
  curAddress.value = selectedAdd.value
...
<template>
  ...
  <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
    <div class="addressWrapper">
      ...
      :class="{active: selectedAdd.id === item.id}">
        <ul>
